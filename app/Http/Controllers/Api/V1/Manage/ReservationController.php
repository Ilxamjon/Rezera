...
use App\Actions\Reservations\MarkReservationPaidAtVenueAction;
...
class ReservationController extends BaseApiController
{
    ...
    public function markPaidAtVenue(
        Business $business,
        Reservation $reservation,
        MarkReservationPaidAtVenueAction $markPaid,
        \Illuminate\Http\Request $request,
    ): JsonResponse {
        $this->ensureBelongsToBusiness($business, $reservation);
        Gate::authorize('updateStatus', $reservation);

        $updated = $markPaid->execute($reservation, $request->user());

        return $this->success(
            new ReservationManagementResource($updated),
            __('payments.marked_paid_at_venue'),
        );
    }
